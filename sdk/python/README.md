...
本 SDK 只用于由 BORING 控制台私有 Runner 启动的本地扩展。正式用户不需要安装
...
控制台源码和正式安装包均通过私有 Runner 提供 SDK；扩展不得调用用户系统 Python。
...
只安装 wheel 不会启动本地 API，也不会提供完整控制台。宿主版本边界见 [兼容说明](../../docs/versions.md)。完整开发包的构建方法见 [开发流程](../../docs/development-guide.md)。
