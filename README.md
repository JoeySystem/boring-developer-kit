...
- **查看或修改源码：**[开发包 v0.1.0-preview.4](https://github.com/JoeySystem/boring-developer-kit/releases/tag/v0.1.0-preview.4)。提供基于 Console 0.1.13 的控制台源码、Python SDK 1.0.0、三个示例、协议与文档，采用非商业许可。

安装包与公开源码本轮均对应 0.1.13；公开源码保留系统字体、独立设置和本地服务等分发适配，与官方安装包外观和标识有所区别。

[开始使用](START-HERE.md) · [下载源码开发包](https://github.com/JoeySystem/boring-developer-kit/releases/tag/v0.1.0-preview.4) · [兼容与验证边界](docs/versions.md) · [许可](LICENSING.md)
...
本次控制台源码取自已提交的 0.1.13 快照，经过公开分发适配，名称为 **BORING Console Community**；它不代表仍在开发的更新产品版本。可以按 [控制台说明](console/README.md) 从源码运行，也可使用兼容的官方宿主。SDK / API 版本不变，配套固件参考仍是 `.07 / sample-verified`。源码开发包不附安装包或固件；官方 0.1.13 安装包单独提供，见 [安装说明](docs/install-console.md)。历史官方安装版验收不能替代公开源码构建验收。具体结果见 [发布验证](docs/verification.md)。
