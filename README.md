...
**固件源码已公开：[编译入口](firmware/README.md) · [DIY 可改、慎改与禁止改动范围](firmware/DIY-GUIDE.md)。USB、认证、更新、启动、分区、电源和存储底层坚决不要改。只有设备仍可正常连接、认证和更新时，才具备控制台恢复官方固件的前提。**
...
- **查看或修改源码：**[开发包 v0.1.0-preview.6](https://github.com/JoeySystem/boring-developer-kit/releases/tag/v0.1.0-preview.6)。新增当前 Power V2 固件源码及 DIY 说明；保留基于 Console 0.1.24 的控制台源码、Python SDK 1.0.0、三个示例和协议，采用非商业许可。

**源码已同步到 0.1.24，公开安装包仍为 0.1.13。** 本次只更新源码开发包；新源码保留系统字体、独立设置和本地服务，使用简化控件示意，不包含官方品牌动画或产品模型。

[开始使用](START-HERE.md) · [下载源码开发包](https://github.com/JoeySystem/boring-developer-kit/releases/tag/v0.1.0-preview.6) · [兼容与验证边界](docs/versions.md) · [许可](LICENSING.md)
