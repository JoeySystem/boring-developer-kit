...
- **查看或修改源码：**[开发包 v0.1.0-preview.8](https://github.com/JoeySystem/boring-developer-kit/releases/tag/v0.1.0-preview.8)。固件源码对齐 Power V2 当前 `20260914.03 / sample-verified` 权威参考；控制台公开适配源码已同步到 0.1.46，同时保留 Python SDK 1.0.0、三个示例和协议，采用非商业许可。

**控制台公开源码为 0.1.46，公开安装包仍为 0.1.13；固件源码参考为 20260914.03。** 本次只更新源码开发包；控制台公开源码保留系统字体、独立设置和本地服务，使用简化控件示意，不包含官方图标、品牌动画、产品模型、官方更新地址或发布记录。

[开始使用](START-HERE.md) · [下载源码开发包](https://github.com/JoeySystem/boring-developer-kit/releases/tag/v0.1.0-preview.8) · [兼容与验证边界](docs/versions.md) · [许可](LICENSING.md)
...
本次控制台源码同步到 2026-09-15 验收的 0.1.46 功能状态，经过公开分发适配，程序标识为 **BORING Console Community**。可以按 [控制台说明](console/README.md) 从源码运行，也可使用兼容的官方宿主。SDK / API 版本不变。新增蓝牙名称功能需要固件声明相应能力；新旧固件及维护包限制见兼容说明。源码开发包含固件源码，不附新 DMG / EXE 或预编译固件；官方 0.1.13 安装包单独提供，见 [安装说明](docs/install-console.md)。历史官方安装版验收不能替代公开源码构建验收。具体结果见 [发布验证](docs/verification.md)。
