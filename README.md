...
- **查看或修改源码：**[开发包 v0.1.0-preview.5](https://github.com/JoeySystem/boring-developer-kit/releases/tag/v0.1.0-preview.5)。提供基于 Console 0.1.24 的控制台源码、Python SDK 1.0.0、三个示例、协议与文档，采用非商业许可。

**源码已同步到 0.1.24，公开安装包仍为 0.1.13。** 本次只更新源码开发包；新源码保留系统字体、独立设置和本地服务，使用简化控件示意，不包含官方品牌动画或产品模型。

[开始使用](START-HERE.md) · [下载源码开发包](https://github.com/JoeySystem/boring-developer-kit/releases/tag/v0.1.0-preview.5) · [兼容与验证边界](docs/versions.md) · [许可](LICENSING.md)
...
本次控制台源码取自已提交的 0.1.24 快照，经过公开分发适配，程序标识为 **BORING Console Community**。可以按 [控制台说明](console/README.md) 从源码运行，也可使用兼容的官方宿主。SDK / API 版本不变。新增蓝牙名称功能需要固件声明相应能力；新旧固件及维护包限制见兼容说明。源码开发包不附安装包或固件；官方 0.1.13 安装包单独提供，见 [安装说明](docs/install-console.md)。历史官方安装版验收不能替代公开源码构建验收。具体结果见 [发布验证](docs/verification.md)。
