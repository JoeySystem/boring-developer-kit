...
- 本轮公开基于已提交 0.1.6 快照的控制台工程，包含公开分发适配；不包含正在开发的 0.1.10。来源与差异见 [控制台源码说明](console-source.md)。
- 可使用本仓库源码运行社区控制台，也可使用兼容官方宿主。SDK wheel 单独安装不会启动宿主或 Runner。
- 不附官方 DMG / EXE 或固件镜像，`.07` 仍通过官方维护方式获取。`sample-verified` 表示已有范围内的留样机验证，不是全部硬件和场景的无条件稳定承诺。
