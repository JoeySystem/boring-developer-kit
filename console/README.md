...
macOS 社区版控制台连接到支持 `codex_usage_display` 能力的 MIST Matrix12 Power V2 固件后，在首页 Codex 额度卡片勾选“在设备屏幕显示额度”。控制台每分钟读取本机 Codex app-server 的七天额度，通过当前已认证的 USB 或蓝牙配置连接发送；屏幕仅在 NORMAL/CODEX 空闲首页显示七天剩余百分比。失去新鲜数据或关闭选项时清除，十分钟未收到更新也会自动恢复原首页。此功能不会修改按键、语音、配置方案或蓝牙设置。
